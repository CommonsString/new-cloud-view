...
<script>
import { getEditIndex } from '@/api/uedeitor'
...
export default {
  ...
  mounted () {
    ...
    console.log('readOptions', this.readOptions)
    console.log('typeOptions', this.typeOptions)
    let obj = this.$store.getters.row
    this.formInline.title = obj.name
    this.msg = obj.content
    this.formInline.read = obj.tag
  ...
  methods: {
    ...
    checkMsg() {
      this.msgObj = this.$store.getters.row
      this.msgObj.name = this.formInline.title
      this.msgObj.tag = this.formInline.read
      this.msgObj.content = this.msg
      this.msgObj.type = this.formInline.type
      this.$confirm('请仔细检查新闻标题、内容', '提示', {
        confirmButtonText: '直接保存',
        cancelButtonText: '返回检查',
        type: 'warning'
      }).then(() => {
          console.log('context', this.msgObj)
          
          this.$store.dispatch('SaveContent', this.msgObj).then(() => {
            this.$message({
              type: 'success',
              message: '保存成功!'
            });
